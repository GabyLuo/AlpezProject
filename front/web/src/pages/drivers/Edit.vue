        ...
        <div class="col q-pa-md">
          ...
          <div class="row q-col-gutter-xs">
            ...
            <div class="col-xs-12 col-sm-3 text-center">
              ...
              >
                <template v-slot:prepend>
                  <q-icon :name="(driver.fields.active.value ? 'battery_full' : 'battery_alert')" />
                </template>
              </q-select>
            </div>
          </div>
          <div class="col-xs-12 col-sm-6 pull-right">
              <q-btn color="positive" icon="save" label="Guardar" @click="updateDriver()"/>
            </div>
        </div>
      </div>
      <!-- <br>
...
export default {
  ...
  methods: {
    async fetchFromServer () {
      this.$q.loading.show()
      api.get(`/drivers/${this.$route.params.id}`).then(async ({ data }) => {
        if (!data.driver) {
          this.$router.push('/drivers')
        } else {
          const postal = data.driver.postal_code_id
          const suburb = data.driver.suburb_id
          this.driver.fields = data.driver
          this.driver.fields.municipio_id = null
          await this.getPostalCodes(data.postal_code)
          this.driver.fields.postal_code_id = postal
          await this.getSuburbsByPostalCode(postal)
          this.driver.fields.suburb_id = suburb
          this.$q.loading.hide()
        }
      })
    },
    updateDriver () {
      ...
      api.put(`/drivers/${this.$route.params.id}`, params).then(({ data }) => {
        this.$q.notify({
