      ...
      <div class="row">
        ...
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white" icon="add" label="Nuevo" @click.native="$router.push('/departments/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'name', align: 'center', label: 'DEPARTAMENTOS', field: 'name', sortable: true },
  ...
  methods: {
    ...
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar este Departamento?',
        persistent: true,
        ok: { label: 'Aceptar', color: 'positive' },
        cancel: { label: 'Cancelar', color: 'negative' }
      }).onOk(() => {
        api.delete(`/departments/${id}`).then(({ data }) => {
          this.$q.notify({
