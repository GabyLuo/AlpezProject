<template>
  <q-page>
        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Auditoria de recepciones" />
            </q-breadcrumbs>
          </div>
        </div>
    ...
    <div class="q-pa-md bg-grey-3">
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="productos" :props="props">{{ props.row.productos }}</q-td>
                <q-td key="entrada" :props="props">{{ props.row.entrada }}</q-td>
                <q-td key="deuda" :props="props">{{ props.row.deuda }}</q-td>
                <q-td key="status" :props="props"><q-chip square dense :color="colorStatus[props.row.status]" text-color="white">{{ props.row.status }}</q-chip></q-td>
                <q-td key="actions" style="width: 10%;" :props="props">
                  <q-btn v-if="props.row.status === 'PENDIENTE'" color="primary" icon="done" flat @click.native="autorizar(props.row.id)" size="10px">
                    <q-tooltip content-class="bg-primary">Autorizar</q-tooltip>
                  </q-btn>
                </q-td>
              </q-tr>
            </template>
      ...
      <q-dialog v-model="shipmentDetailsModal" persistent>
        <q-card style="width: 1000px; height: 1000px">
          <q-card-section>
            <q-toolbar>
              <q-toolbar-title>Detalles de orden de compra</q-toolbar-title>
              <q-btn flat v-close-popup round dense icon="close" />
            </q-toolbar>
          </q-card-section>

          <q-card-section style="width: 1000px; height: 1000px">
                  ...
                  >
                    <template v-slot:body="props">
                      <q-tr :props="props">
                        ...
                        <q-td key="qty" style="text-align: center; width: 5%;" :props="props">{{ props.row.qty }}</q-td>
                        <q-td key="price" style="text-align: center; width: 5%;" :props="props">{{ `${currencyFormatter.format(props.row.price)}` }}</q-td>
                        <q-td key="totalPrice" style="text-align: center; width: 5%;" :props="props">{{ `${currencyFormatter.format(props.row.price * props.row.qty)}` }}</q-td>
                        <q-td key="entrada" style="text-align: center; width: 5%;" :props="props">{{ props.row.entrada }}</q-td>
                        <q-td key="restante" style="text-align: center; width: 5%;" :props="props">{{ props.row.restante }}</q-td>
                        ...
                        <q-td key="actions" style="width: 10%;" :props="props" v-if="order.fields.status != 'APROBADO' && order.fields.status != 'CERRADO' && order.fields.status != 'CANCELADO'">
                          ...
                          <q-btn color="primary" icon="fas fa-trash-alt" flat @click.native="removeDetail(props.row)" size="10px">
          ...
          <q-card-actions align="right" class="text-primary">
            <q-btn flat label="Autorizar" @click="updateDetail()" />
          </q-card-actions>
        </q-card>
      </q-dialog>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'serial', align: 'center', label: '# Orden', field: 'serial', sortable: true },
      ...
      detailsColumns: [
        ...
        { name: 'qty', align: 'center', label: 'Cantidad', field: 'qty', style: 'width: 5%', sortable: true },
        { name: 'price', align: 'center', label: 'Precio unitario', field: 'price', style: 'width: 5%', sortable: true },
        { name: 'totalPrice', align: 'center', label: 'Importe', field: 'totalPrice', style: 'width: 5%', sortable: true },
  ...
  methods: {
    ...
    async qTableRequest (props) {
      ...
      await api.post('/shipments/report', params).then(({ data }) => {
        this.$q.loading.hide()
        console.log(data.shipments)
        this.data = data.shipments
        this.pagination.rowsNumber = data.shipmentsCount
      }).catch(error => error)
    },
    getClients () {
    ...
    autorizar (id) {
      const params = { id: id }
      api.put('/purchase-orders/autorizar', params).then(({ data }) => {
        this.$q.notify({
          message: 'Se ha autorizado la recepción',
          position: 'top',
          color: (data.result ? 'positive' : 'warning')
        })
        this.fetchFromServer()
