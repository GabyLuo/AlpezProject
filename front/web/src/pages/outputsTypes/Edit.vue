        ...
        <div class="col-sm-9">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Ruberos" to="/categories" />
              <q-breadcrumbs-el label="Editar Rubro" v-text= "output.fields.name"/>
            </q-breadcrumbs>
          </div>
        </div>
        <!-- <div class="col-sm-3">
        ...
        <div class="col q-pa-md">
          ...
          <div class="row q-mb-sm q-mt-md">
            <div class="col-xs-12 col-sm-2 offset-sm-10 pull-right">
              <q-btn color="positive" icon="save" label="Actualizar" @click="editOutput()" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      ...
      api.get(`/output_type/${id}`).then(({ data }) => {
        if (!data.output) {
          this.$router.push('/outputs-types')
        } else {
          this.output.fields = data.output
          this.$q.loading.hide()
        }
      })
    },
    ...
    editOutput () {
      ...
      api.put(`/output_type/${params.id}`, params).then(({ data }) => {
        this.$q.notify({
