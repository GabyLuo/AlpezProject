      ...
      <div class="row">
        ...
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white" icon="add" label="Nuevo" @click.native="$router.push('/outputs-types/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/output_type').then(({ data }) => {
        this.data = data.outputs
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
      this.$router.push(`/outputs-types/${id}`)
    },
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar esta Rubro?',
        persistent: true,
        ok: {
          label: 'Aceptar',
          color: 'green'
        ...
        api.delete(`/output_type/${id}`).then(({ data }) => {
          this.$q.notify({
