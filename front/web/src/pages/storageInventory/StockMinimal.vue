        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Stock Minimo" />
            </q-breadcrumbs>
          </div>
        </div>
        ...
        <div class="col q-pa-md">
          ...
          >
            <template v-slot:body="props">
              <q-tr :props="props">
                <q-td key="category" style="text-align: left; width: 20%;" :props="props">{{ props.row.category }}</q-td>
                <q-td key="line" style="text-align: left; width: 20%;" :props="props">{{ props.row.line }}</q-td>
                ...
                <q-td key="branch_office_name" style="text-align: left; width: 20%;" :props="props">{{ props.row.branch_office_name }}</q-td>
                <q-td key="storage_name" style="text-align: left; width: 20%;" :props="props">{{ props.row.storage_name }}</q-td>
                <q-td key="marca" style="text-align: left; width: 20%;" :props="props">{{ props.row.marca }}</q-td>
                <q-td key="product" style="text-align: left; width: 40%;" :props="props">{{ props.row.product_name }}</q-td>
                <q-td key="minimal_stock" style="text-align: right; width: 10%" :props="props">
                  <label >{{ formatPrice(props.row.minimal_stock) }} </label></q-td>
                <q-td key="stock" v-if="(props.row.minimal_stock) <= props.row.stock" style="text-align: right; width: 10%; background-color:#21BA45; color:white" :props="props">
                  <label >{{ formatPrice(props.row.stock) }} </label></q-td>
                <q-td key="stock" v-else style="text-align: right; width: 10%; background-color:#C10015; color:white" :props="props">
                  <label >{{ formatPrice(props.row.stock) }} </label></q-td>
              </q-tr>
            </template>
          </q-table>
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'branch_office_name', align: 'center', label: 'ESTACIÓN', field: 'branch_office_name', style: 'width: 10%', sortable: true },
        { name: 'storage_name', align: 'center', label: 'ALMACÉN', field: 'storage_name', style: 'width: 10%', sortable: true },
        { name: 'marca', align: 'center', label: 'MARCA', field: 'marca', style: 'width: 10%', sortable: true },
        { name: 'product', align: 'center', label: 'PRODUCTO', field: 'product', style: 'width: 40%', sortable: true },
        { name: 'minimal_stock', align: 'center', label: 'STOCK MINIMO', field: 'minimal_stock', style: 'width: 10%', sortable: true, sort: (a, b) => Number(a, 10) - Number(b, 10) },
  ...
  methods: {
    filterMarcas (val, update, abort) {
      update(() => {
        ...
        this.optionsmarks = this.markOptions.filter(v => v.label.toLowerCase().indexOf(needle) > -1)
      })
    },
    ...
    fetchFromServer () {
      ...
      if (branch === 9 && propiedades !== 1) {
        this.branchOffice = { value: 9, label: 'MATRIZ' }
      }
      if (branch === 12 && propiedades !== 1) {
        this.branchOffice = { value: 12, label: 'LOPEZ DE LARA TINAJERO GUILLERMO' }
      }
      this.$q.loading.show()
      this.qTableRequest({
    ...
    async qTableRequest (props) {
      ...
      params.mark = mark
      await api.post('/movements/storageInventoryMinimal', params).then(({ data }) => {
        console.log(data)
        this.$q.loading.hide()
    ...
    searchStock () {
      ...
      api.get(`/movements/storageInventoryMinimal/${branchOfficeId}/${storageId}/${categoryId}/${lineId}/${productId}/${date}`).then(({ data }) => {
        if (data.result) {
    ...
    generatePDF () {
      ...
      const uri = process.env.API + `movements/inventory-minimal-stock/pdf/${branchOfficeId}/${storageId}/${categoryId}/${lineId}/${productId}/${user}/${mark}`
    ...
    generateCsv () {
      ...
      const uri = process.env.API + `movements/inventory-minimal-stock/csv/${branchOfficeId}/${storageId}/${categoryId}/${lineId}/${productId}/${user}/${mark}`
      window.open(uri, '_blank')
    }
  }
}
