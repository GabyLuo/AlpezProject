        ...
        <div class="col-sm-9">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Editar tipos de vehículo" v-text= "vehicle.fields.economic_number"/>
            </q-breadcrumbs>
          </div>
        </div>
      </div>
        ...
        <div class="col q-pa-md">
          ...
          <div class="row q-mb-sm q-mt-md">
            <div class="col-xs-12 col-sm-2 offset-sm-10 pull-right">
              <q-btn color="positive" icon="save" label="Guardar" @click="editVehicle()" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      ...
      api.get(`/vehicle/${id}`).then(({ data }) => {
        if (!data.vehicle) {
          this.$router.push('/vehicle')
        } else {
          this.vehicle.fields = data.vehicle
          this.vehicle.fields.vehicle_config = data.vehicle.vehicle_config
          this.$q.loading.hide()
        }
      })
    },
    editVehicle () {
      ...
      api.put(`/vehicle/${params.id}`, params).then(({ data }) => {
        this.$q.notify({
    ...
    getVehicleType () {
      api.get('vehicleType/options').then(data => {
        ...
        this.vehicleTypeFilter = data.data.options
      })
    },
    getTowingType () {
    ...
    getConfig () {
      api.get('vehicleType/optionsconfig').then(data => {
        ...
        this.vehicleConfigFilter = data.data.options
      })
    },
