<template>
  <q-page>
    ...
    <div class="q-pa-sm panel-header">
      <div class="row">
        <div class="col-sm-8">
          <span class="q-ml-md grey-8 fs28 page-title">Pedidos solicitados</span>
        </div>
        <div class="col-sm-4 pull-right">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="right">
              <q-breadcrumbs-el label="" icon="home" to="/" />
              <q-breadcrumbs-el label="Pedidos solicitados" />
            </q-breadcrumbs>
          </div>
        </div>
      </div>
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="actions" :props="props">
                  ...
                  <q-btn color="primary" icon="edit" flat @click.native="changeStatusRow(props.row.id)" size="10px">
...
export default {
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/shopping-carts/requested').then(({ data }) => {
        this.data = data.orders
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
      this.$router.push(`/requested-orders/${id}`)
    },
    changeStatusRow (id) {
      const params = []
      params.cartId = Number(id)
