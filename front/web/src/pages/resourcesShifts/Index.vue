        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Turnos" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white" icon="add" label="Nuevo" @click.native="$router.push('/shifts/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'name', align: 'center', label: 'TURNOS', field: 'name', sortable: true },
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/shifts').then(({ data }) => {
        this.data = data.shifts
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
