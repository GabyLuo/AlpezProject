        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Destinos" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn color="primary" icon="add" label="Nuevo" @click.native="$router.push('/ranges/new')" />
          </div>
        </div>
      </div>
    </div>
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="municipality" style="text-align: left;" :props="props">{{ props.row.municipality }}</q-td>
                <q-td key="distance" style="text-align: center;" :props="props">{{ props.row.distance+' km.' }}</q-td>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/ranges').then(({ data }) => {
        this.data = data.ranges
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
      this.$router.push(`/ranges/${id}`)
    },
    deleteSelectedRow (id) {
      this.$q.dialog({
        ...
        api.delete(`/ranges/${id}`).then(({ data }) => {
          this.$q.notify({
