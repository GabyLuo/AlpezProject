        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Días festivos" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white" icon="add" label="Nuevo" @click.native="$router.push('/holidays/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'note', align: 'center', label: 'NOTA', field: 'note', sortable: true },
  ...
  methods: {
    ...
    editSelectedRow (id) {
      this.$router.push(`/holidays/${id}`)
    },
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar este día festivo?',
        persistent: true,
        ok: { label: 'Aceptar', color: 'positive' },
        cancel: { label: 'Cancelar', color: 'negative' }
      }).onOk(() => {
        api.delete(`/holidays/${id}`).then(({ data }) => {
          this.$q.notify({
