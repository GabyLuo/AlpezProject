  ...
  <q-page class="bg-grey-3">
    <div class="q-pa-sm panel-header">
      <div class="row q-col-gutter-md">
        ...
        <div class="col-sm-9 pull-right">
          ...
          <q-btn style="margin-left: 8px;" v-if="cartStatus !== 'NUEVO' && (this.role === 1 || this.role === 20 || this.role === 17)" color="purple" icon="mail" @click="confirmSendPDF()">
            <q-tooltip content-class="bg-positive">Enviar email</q-tooltip>
          </q-btn>
          <q-btn v-if="cartStatus === 'AUTORIZADO' || cartStatus === 'REMISIONADO' || cartStatus === 'PARCIAL' && (this.role === 1 || this.role === 20 || this.role === 17)" style="margin-left: 8px;" color="red" icon="fas fa-file-pdf" @click="downloadPDF()">
            <q-tooltip content-class="bg-positive">Descargar PDF</q-tooltip>
          </q-btn>
          <q-btn v-if="cartStatus === 'SOLICITADO' || cartStatus === 'AUTORIZADO' && (this.role === 1 || this.role === 20 || this.role === 23)" style="margin-left: 8px;" color="red" icon="block" label="CANCELAR" @click="cancel()" />
          <q-btn v-if="cartStatus === 'SOLICITADO' || cartStatus === 'AUTORIZADO' && (this.role === 1 || this.role === 20 || this.role === 23)" style="margin-left: 8px;" color="warning" icon="edit" label="Modificar" @click="confirm()" />
          ...
          <q-btn v-if="cartStatus === 'SOLICITADO' && (this.role === 1 || this.role === 23 || this.$store.getters['users/roles'].includes(25))" style="margin-left: 8px;" color="positive" icon="check" label="Aprobar" @click="approve()" />
          <q-btn v-if="cartStatus === 'AUTORIZADO' || cartStatus === 'PARCIAL' && (this.role === 1 || this.role === 2)"  style="margin-left: 8px;" :disable="this.role != 1 && this.role != 2" color="positive" icon="check" label="REMISIONAR" @click="openGenerateInvoiceModal()" />
          </div>
        </div>
      </div>
    </div>
    <div class="q-pa-md bg-grey-3">
      <div class="row bg-white border-panel">
        <div class="col q-pa-md">
          <div class="row q-col-gutter-xs">
            ...
            <div class="col-xs-12 col-sm-10">
              <q-input
                ...
                filled
                autogrow
                :disable="cartStatus !== 'SOLICITADO' && (this.role !== 1 || this.role !== 20 || this.roles != 23)"
          ...
          <div v-if="cartStatus === 'SOLICITADO'" class="q-pa-md">
              ...
              <template v-slot:body="props">
                <q-tr :props="props">
                  ...
                  <q-td key="status" :props="props" style="width:5%;">
                    <q-chip square dense :color="props.row.status == 'REMISIONADO' ? 'positive' : (props.row.status == 'AUTORIZADO' ? 'warning' : (props.row.status == 'POSTERGADO' ? 'deep-purple-7' : (props.row.status == 'CANCELADO' ? 'red' : 'primary')))" text-color="white">
                      {{ props.row.status }}
                    </q-chip>
                  </q-td>
                </q-tr>
        ...
        <div style="width:400px;">
          ...
          <div class="row" style="margin-top: 15px;" >
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
              <div class="row">
                <div class="col-md-12 q-pl-md" style="text-align: left;">
    ...
    <q-dialog v-model="confirmModalPDF" persistent>
      <q-card style="width:500px;">
        ...
        <br>
        <q-card-section class="row items-center">
          <label style="font-size: 23px;">¿Desea enviar el PDF del pedido al cliente?</label>
        </q-card-section>
        <br><br>
        <q-card-actions align="right" v-if="buttonModal">
          <q-btn label="Cancelar" color="negative" v-close-popup />
...
export default {
  ...
  computed: {
    ...
    totalPrice () {
      ...
      let subtotal = 0
      this.shoppingCartInBulkDetails.forEach(detail => {
        price = Number(detail.amount) / 1.16 / (Number(detail.ieps) / 100 + 1)
        subtotal += price
      })
    ...
    thisIva () {
      ...
      let totalIva = 0
      this.shoppingCartInBulkDetails.forEach(detail => {
        price = Number(detail.amount) - (Number(detail.amount / 1.16))
        totalIva += price
      })
      return this.currencyFormatter.format(totalIva)
    },
    thisTotal () {
      let price = 0
      this.shoppingCartInBulkDetails.forEach(detail => {
    ...
    generateInvoice () {
      if (this.canGenerateInvoice) {
        ...
        api.put(`/shopping-carts/${this.$route.params.id}/generate-invoice`, params).then(({ data }) => {
          if (data.message2) {
            for (let i = 0; i < Object.keys(data.message2).length; i++) {
              this.$q.notify({
                message: data.message2[i].content,
                position: 'top',
                color: (data.result ? 'positive' : 'warning')
              })
            }
