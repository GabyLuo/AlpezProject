        ...
        <div class="col-sm-9">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Editar unicipio" v-text= "municipality.fields.nombre"/>
            </q-breadcrumbs>
          </div>
        </div>
      </div>
        ...
        <div class="col q-pa-md">
          ...
          <div class="row q-mb-sm q-mt-md">
            <div class="col-xs-12 col-sm-2 offset-sm-10 pull-right">
              <q-btn color="positive" icon="save" label="Guardar" @click="editMunicipality()" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      ...
      api.get(`/municipality/${id}`).then(({ data }) => {
        if (!data.municipality) {
          this.$router.push('/municipality')
        } else {
          this.municipality.fields = data.municipality
          this.municipality.fields.state_id = { value: data.municipality.state_id, label: data.municipality.state_name }
          this.$q.loading.hide()
        }
      })
    },
    editMunicipality () {
      ...
      if (this.municipality.fields.state_id.value) {
        params.state_id = this.municipality.fields.state_id.value
      }
      console.log(params)
      api.put(`/municipality/${params.id}`, params).then(({ data }) => {
        this.$q.notify({
