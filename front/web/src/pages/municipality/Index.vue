        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Municipios" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn color="primary" icon="add" label="Nuevo" @click.native="$router.push('/municipality/new')" />
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="col q-pa-md">
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                <q-td key="state_name" style="text-align: left;" :props="props">{{ props.row.state_name }}</q-td>
                <q-td key="municipality_name" style="text-align: left;" :props="props">{{ props.row.municipality_name }}</q-td>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'state_name', align: 'center', label: 'ESTADO', field: 'state_name', sortable: true },
        { name: 'municipality_name', align: 'center', label: 'MUNICIPIO', field: 'municipality_name', sortable: true },
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/municipality').then(({ data }) => {
        this.data = data.municipalitys
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
      this.$router.push(`/municipality/${id}`)
    },
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar esta Ciudad?',
        persistent: true,
        ok: {
          label: 'Aceptar',
          color: 'green'
        ...
        api.delete(`/municipality/${id}`).then(({ data }) => {
          this.$q.notify({
