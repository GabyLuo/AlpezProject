        ...
        <div class="col-sm-6">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Corte de caja" />
            </q-breadcrumbs>
          </div>
        </div>
                    ...
                    :filter="filter">
                        ...
                        <template v-slot:body="props">
                            <q-tr :props="props">
                                ...
                                <q-td key="folio_fiscal" style="text-align: center;" :props="props" v-if="props.row.name !== ''">{{ props.row.folio_fiscal }}</q-td>
                                <q-td key="folio_fiscal" style="text-align: right;" :props="props" v-else>{{ props.row.total }}</q-td>
                                ...
                                <q-td key="invoice" style="text-align: right;" :props="props" v-if="props.row.name !== ''">$ {{ $formatNumberPrice(props.row.invoice) }}</q-td>
                                <q-td key="invoice" style="text-align: right;" :props="props" v-else>$ {{ $formatNumberPrice(props.row.countinvoice) }}</q-td>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'folio_fiscal', align: 'center', label: 'FISCAL', field: 'folio_fiscal', sortable: false },
        ...
        { name: 'invoice', align: 'center', label: 'FACTURADO', field: 'invoice', sortable: false },
  ...
  methods: {
    filterGrid () {
      ...
      this.$q.loading.show()
      api.get(`/close-sale/getCloseSales/${date}`).then(({ data }) => {
        if (data.result) {
    ...
    fetchFromServer () {
      ...
      this.$q.loading.show()
      api.get(`/close-sale/getCloseSales/${this.getDate}`).then(({ data }) => {
        if (data.result) {
          this.closeSale = data.closeSale
          console.log(this.closeSale)
        }
