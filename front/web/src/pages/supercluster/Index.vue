        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Clusters" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white" icon="add" label="Nuevo" @click.native="$router.push('/supercluster/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/supercluster/getAll').then(({ data }) => {
        this.data = data.supercluster
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
      this.$router.push(`/supercluster/${id}`)
    },
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar este Cluster?',
        persistent: true,
        ok: {
          label: 'Aceptar',
          color: 'green'
        ...
        api.delete(`/supercluster/delete/${id}`).then(({ data }) => {
          this.$q.notify({
