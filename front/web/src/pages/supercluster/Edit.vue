        ...
        <div class="col-sm-9">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el :label="supercluster.fields.name" />
            </q-breadcrumbs>
          </div>
        </div>
      </div>
        ...
        <div class="col q-pa-md">
          ...
          <div class="row q-mb-sm q-mt-md">
            <div class="col-xs-12 col-sm-2 offset-sm-10 pull-right">
              <q-btn color="positive" icon="save" label="Guardar" @click="update()" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      ...
      api.get(`/supercluster/getById/${id}`).then(({ data }) => {
        if (!data.supercluster) {
          this.$router.push('/supercluster')
        } else {
          this.supercluster.fields = data.supercluster[0]
        }
        this.$q.loading.hide()
      })
    },
    update () {
      ...
      api.put(`/supercluster/update/${this.supercluster.fields.id}`, params).then(({ data }) => {
        this.$q.notify({
