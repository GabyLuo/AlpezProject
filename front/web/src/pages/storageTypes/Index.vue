        ...
        <div class="col-sm-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Tipos de Almacén" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white"  icon="add" label="Nuevo" @click.native="$router.push('/storage-types/new')" />
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="col q-pa-md">
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                <q-td key="name" style="width: 85%;text-align: left;" :props="props">{{ props.row.name }}</q-td>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/storage-types').then(({ data }) => {
        this.data = data.storageTypes
        this.$q.loading.hide()
      })
    },
    editSelectedRow (id) {
      this.$router.push(`/storage-types/${id}`)
    },
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar este Tipo de Almacén?',
        persistent: true,
        ok: {
          label: 'Aceptar',
          color: 'green'
        ...
        api.delete(`/storage-types/${id}`).then(({ data }) => {
          this.$q.notify({
