        ...
        <div class="col-xs-12 col-md-8">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left" style="font-size: 20px">
              ...
              <q-breadcrumbs-el label="Paqueterías" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-xs-12 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white"  icon="add" label="Nuevo" @click.native="$router.push('/carriers/new')" />
          </div>
        </div>
      </div>
    </div>
          ...
          >
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="url" style="text-align: left;" :props="props">{{ props.row.url }}</q-td>
...
export default {
  ...
  methods: {
    ...
    deleteSelectedRow (id) {
      this.$q.dialog({
        title: 'Confirmación',
        message: '¿Desea eliminar esta Paquetería?',
        persistent: true,
        ok: {
          label: 'Aceptar',
          color: 'green'
        ...
        api.delete(`/carriers/${id}`).then(({ data }) => {
          this.$q.notify({
