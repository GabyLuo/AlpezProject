        ...
        <div class="col-sm-9"  style="font-size: 20px">
          <div class="q-pa-md q-gutter-sm">
            <q-breadcrumbs align="left">
              ...
              <q-breadcrumbs-el label="Editar Subcategoría" v-text="line.fields.name" />
            </q-breadcrumbs>
          </div>
        </div>
        <div class="col-sm-3">
        ...
        <div class="col q-pa-md">
          ...
          <div class="row q-col-gutter-xs">
            <div class="col-xs-12 col-sm-4">
              <q-select
                ...
                emit-value
                map-options
                v-model="line.fields.category_id"
                :options="categoryOptions"
                label="Categorías"
                :rules="categoryRules"
                @input="updateLineFields"
              >
          ...
          <div class="row q-mb-sm q-mt-md">
            <div class="col-xs-12 col-sm-2 offset-sm-10 pull-right">
              <q-btn color="positive" icon="save" label="Actualizar" @click="editLine()" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    fetchFromServer () {
      ...
      api.get(`/lines/${id}`).then(({ data }) => {
        if (!data.line) {
          this.$q.loading.hide()
          this.$router.push('/lines')
        } else {
          console.log(data.line)
          this.line.fields = data.line
          this.line.fields.category = data.line.category_id
        }
        this.$q.loading.hide()
        console.log(this.line.fields)
      })
    },
    updateLineFields () {
    ...
    editLine () {
      ...
      this.$q.loading.show()
      const params = { ...this.line.fields }
      api.put(`/lines/${params.id}`, params).then(({ data }) => {
        this.$q.notify({
