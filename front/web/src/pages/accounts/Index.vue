      ...
      <div class="row">
        ...
        <div class="col-xs-6 col-md-4 pull-right">
          <div class="q-pa-sm q-gutter-sm">
            <q-btn class="bg-primary" style="color: white" icon="add" label="Nuevo" @click.native="$router.push('/accounts/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    fetchFromServer () {
      this.$q.loading.show()
      api.get('/account-trade').then(({ data }) => {
    ...
    deleteSelectedRow (id) {
      this.$q.dialog({
        ...
        api.delete(`/account-trade/${id}`).then(({ data }) => {
          this.$q.notify({
