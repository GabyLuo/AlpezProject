...
<div class="col-md-6">
  ...
  <div class="col-md-6">
    ...
    color="dark"
    bg-color="secondary"
    v-model="clientes.fields.seller"
    label="Vendedor"
    :options="options2"
    use-input
    hide-selected
  ...
  <div class="col q-pa-md">
    ...
    <template v-slot:body="props">
      <q-tr :props="props">
        ...
        <q-td key="seller" :props="props">{{ props.row.vendedor }}</q-td>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'seller', align: 'left', label: 'VENDEDOR', field: 'seller', sortable: true },
  ...
  methods: {
    ...
    async qTableRequest (props) {
      ...
      params.seller = this.clientes.fields.seller
      params.cliente = this.clientes.fields.cliente
      params.pagination = this.pagination
      params.filter = this.filter
      // console.log(params)
      api.post('/reports/pagbySellers', params).then(({ data }) => {
        // this.$q.loading.hide()
        this.data = data.info
    ...
    async getOptionsClients () {
      await api.get('/customers/options').then(({ data }) => {
        this.optionsClientsFilter = data.options
        this.optionsClientsFilter.push({ label: 'TODOS', value: 'TODOS' })
      })
    },
    filtrarClientes (val, update, abort) {
      update(() => {
        const needle = val.toLowerCase()
        this.optionsClients = this.optionsClientsFilter.filter(v => v.label.toLowerCase().indexOf(needle) > -1)
      })
    },
    filtrarSeller (val, update, abort) {
      update(() => {
        const needle = val.toLowerCase()
    ...
    getClients () {
      api.get('/users/sellers').then(({ data }) => {
        this.customerOptions = data.sellers
    ...
    generateCSV () {
      ...
      if (this.clientes.fields.seller != null) {
        params.seller = this.clientes.fields.seller.value
      } else {
        params.seller = null
      }
      ...
      const uri = process.env.API + `reports/getCsvpagbySeller/${params.DateOf}/${params.DateUntil}/${params.sucursal}/${params.marca}/${params.product}/${params.linea}/${params.seller}/${params.cliente}`
      ...
      api.fileDownload(uri).then(({ data }) => {
        ...
        link.href = url
        link.setAttribute('download', 'ReporteVendedor.csv')
    ...
    generatePDF () {
      ...
      // console.log(params.cliente)
      const uri = process.env.API + `reports/getPdfpagbySeller/${params.DateOf}/${params.DateUntil}/${params.sucursal}/${params.marca}/${params.product}/${params.linea}/${params.seller}/${params.cliente}`
      // window.open(uri, '_blank')
      this.$q.loading.show()
      api.fileDownload(uri).then(({ data }) => {
        ...
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', 'ReporteVendedor.pdf')
        document.body.appendChild(link)
        this.$q.loading.hide()
        link.click()
      })
