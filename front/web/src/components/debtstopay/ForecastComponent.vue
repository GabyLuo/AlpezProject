        ...
        <template #day="{ timestamp }">
          <template v-for="(event, index) in getEvents(timestamp.date)">
            <q-badge
              ...
              <q-tooltip transition-show="scale" transition-hide="scale" :content-class="event.bgcolor" content-style="font-size: 14px">PLAZO</q-tooltip>
            </q-badge>
          </template>
        </template>
      </q-calendar>
      ...
      <q-card style="min-width: 70%;max-height: 90vh">
        <div>
          <div class="bg-primary">
            <div class="row">
              <div class="col-sm-11 text-h6" style="color:white;padding-top: 5px; padding-bottom: 5px;">&nbsp;&nbsp; DETALLES DEUDAS POR PAGAR</div>
        ...
        <q-tabs v-model="currentTab" inline-label dense class="text-grey" active-color="primary" indicator-color="primary" align="justify" narrow-indicator @input="changeModel">
          <q-tab name="client" icon="person" label="POR PROVEEDOR" />
          <q-tab name="rem" icon="outbox" label="POR ORDEN" />
        </q-tabs>
        <q-tab-panels v-model="currentTab" animated>
          <q-tab-panel name="client">
            <div class="row q-pa-md">
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <q-table style="max-height:80vh" color="primary" :data="info" :columns="infoColumnsClient" row-key="id" :pagination.sync="pagination">
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td  class="pull-left" key="customer_name" style="text-left: center; width:15%;" :props="props"><label> {{ props.row.name_sup }}</label></q-td>
                      <q-td key="contador" style="text-align: center; width:15%;" :props="props"><label> {{ props.row.qtyorders }}</label></q-td>
                      <q-td key="total" style="text-align: right; width:15%;" :props="props"><label>$ {{ formatPrice(props.row.total) }}</label></q-td>
                      <q-td key="paid" style="text-align: right; width:15%;" :props="props"><label>$ {{ formatPrice(props.row.abonado) }}</label></q-td>
            ...
            <div class="row q-pa-md">
              <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <q-table style="max-height:60vh; width: 100%;" color="primary" :data="info" :columns="infoColumnsRem" row-key="id" :pagination.sync="pagination">
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td  key="rem" style="text-align: center; width:10%;" :props="props"><label> {{ props.row.serial }}</label></q-td>
                      <q-td  class="pull-left" key="customer_name" style="text-align: left; width:25%;" :props="props"><label> {{ props.row.name_suppliers }}</label></q-td>
                      <q-td key="total" style="text-align: right; width:15%;" :props="props"><label>$ {{ formatPrice(props.row.totalamount) }}</label></q-td>
                      <q-td key="paid" style="text-align: right; width:15%;" :props="props"><label>$ {{ formatPrice(props.row.abonado) }}</label></q-td>
                      <q-td key="remaining" style="text-align: right; width:15%;" :props="props">
                        <q-badge transparent  class="bg-red-14">
                          <label style="font-size: 13px;font-weight: bold;">$ {{ formatPrice(props.row.restante) }}</label>
                        </q-badge>
                      </q-td>
                    </q-tr>
                  </template>
...
export default {
  ...
  data () {
    return {
      ...
      info: [],
      infoColumnsClient: [
        { name: 'customer_name', align: 'center', label: 'PROVEEDOR', field: 'customer_name', sortable: true },
        { name: 'contador', align: 'center', label: 'CANT. ORDENES', field: 'contador', sortable: true },
      ...
      infoColumnsRem: [
        { name: 'rem', align: 'center', label: 'ORDEN', field: 'rem', sortable: true },
        { name: 'customer_name', align: 'center', label: 'PROVEEDOR', field: 'customer_name', sortable: true },
        { name: 'total', align: 'center', label: 'TOTAL', field: 'total', sortable: true },
        { name: 'paid', align: 'center', label: 'ABONADO', field: 'paid', sortable: true },
        { name: 'remaining', align: 'center', label: 'RESTANTE', field: 'remaining', sortable: true }
  ...
  methods: {
    ...
    ...mapActions({
      getDataCalendar: 'debts/forecast/dataC'
    }),
    loadAll () {
      this.$q.loading.show()
      this.getDataCalendar({ branch_id: this.branche }).then(({ data }) => {
    ...
    getDataClient (date, tipo) {
      ...
      params.date = date
      params.type = tipo
      params.branch_id = this.branche
      api.post('/forecast-debts/getDebtsToPay', params).then(({ data }) => {
        this.info = data.data
        this.total = data.sm
        this.$q.loading.hide()
      })
    ...
    getDataRem (date, tipo) {
      ...
      params.type = tipo
      params.branch_id = this.branche
      api.post('/forecast-debts/getDebtsToPayForSuppliers', params).then(({ data }) => {
        this.info = data.data
